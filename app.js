...
function renderKeyboardHero() {
  ...
  return `
    <section class="hero card reveal">
      <div class="hero-copy">
        <p class="eyebrow">Russian typing practice</p>
        <h1>Russian Key Coach</h1>
        <p class="hero-text">
          Train one letter at a time. The app tracks reaction time, wrong-key count,
          and keeps permanent progress on your server so every device sees the same history.
        </p>
        ${renderModuleTabs()}
      ...
      <div class="hero-metrics">
        ...
        <div class="metric-block">
          ...
          <span class="metric-detail">${escapeHtml(saveStatus.label)} across devices.</span>
        </div>
      </div>
    </section>
  `;
...
function renderGrammarHero(moduleId) {
  const moduleDef = GRAMMAR_MODULES[moduleId];
  const moduleProgress = getGrammarModuleProgress(moduleId);
  const session = getGrammarSession(moduleId);
  ...
  const weakestPerson = PERSONS.map((person) => {
    const personAtoms = focusAtomIds.filter(
      (atomId) => moduleDef.atomsById[atomId].personId === person.id,
    );
    const score = personAtoms.length
      ? average(
          personAtoms,
          (atomId) => buildGrammarAtomProfile(moduleId, atomId).displayScore,
        )
      : 0;
    return { person, score };
  ...
  return `
    <section class="hero card reveal">
      <div class="hero-copy">
        <p class="eyebrow">Russian conjugation practice</p>
        <h1>${escapeHtml(moduleDef.title)}</h1>
      ...
      <div class="hero-metrics">
        <div class="metric-block">
          ...
          <span class="metric-detail">Tracked per atom and per stage in the active subdeck.</span>
        </div>
        <div class="metric-block">
          ...
          <span class="metric-detail">Weakest person: ${escapeHtml(weakestPerson?.person.pronoun || "—")}.</span>
        </div>
      </div>
    </section>
  `;
...
function renderHero() {
  if (appState.activeTab === "keyboard") {
    return renderKeyboardHero();
  }

  if (GRAMMAR_MODULES[appState.activeTab]) {
    return renderGrammarHero(appState.activeTab);
  }
...
function renderGrammarPromptBody(moduleId, atom, prompt, canInteract) {
  ...
  return `
    <div class="target-pronoun-chip">${escapeHtml(atom.pronoun)}</div>
    ${helperBanner}
    ${coachNote}
    <p class="prompt-copy">${escapeHtml(promptCopy)}</p>
    <div class="grammar-stage-body">${body}</div>
  `;
}
  ...
  return `
    <article class="practice card grammar-practice ${session.flashTone === "correct" ? "flash-correct" : session.flashTone === "wrong" ? "flash-wrong" : ""}">
      ...
      <div class="grammar-card-head">
        <div class="grammar-card-copy">
          ${
            atom
              ? `
                <div class="grammar-lemma-title">${escapeHtml(atom.lemma)}</div>
                <div class="grammar-translation">${escapeHtml(atom.translation)}</div>
              `
              : `
                <div class="grammar-lemma-title">${escapeHtml(GRAMMAR_MODULES[moduleId].title)}</div>
                <div class="grammar-translation">Press Resume to start the adaptive flow for the selected subdeck.</div>
              `
          }
        </div>
        <div class="grammar-chip-row">
...
function renderAtomDetail(moduleId, atomId) {
  ...
  return `
    <div class="detail-panel">
      <div class="panel-header">
        <div>
          <p class="section-label">Selected atom</p>
          <h3>${escapeHtml(atom.lemma)} · ${escapeHtml(atom.pronoun)}</h3>
          <p class="list-note">${escapeHtml(atom.answer)}</p>
        </div>
        <span class="chip ${atomProgress.mastered ? "current" : "active"}">
      ...
      <div class="detail-grid">
        <div class="detail-item">
          ...
          <strong class="detail-value">${atomProgress.attempts}</strong>
        </div>
        <div class="detail-item">
          <span class="detail-label">Average ms</span>
          <strong class="detail-value">${displayMs(lifetimeAvgTime)}</strong>
        </div>
        <div class="detail-item">
          <span class="detail-label">Recent ms</span>
          <strong class="detail-value">${displayMs(recentAvgTime)}</strong>
        </div>
        <div class="detail-item">
          <span class="detail-label">Avg errors</span>
          <strong class="detail-value">${displayErrors(lifetimeAvgErrors)}</strong>
        </div>
        <div class="detail-item">
          <span class="detail-label">Recent errors</span>
          <strong class="detail-value">${displayErrors(recentAvgErrors)}</strong>
        </div>
        <div class="detail-item">
...
function renderGrammarAnalyticsCard(moduleId) {
  ...
  return `
    <article class="session card grammar-analytics">
      <div class="session-header">
        <div>
          <p class="section-label">Analytics</p>
          <h2>Local weakness stays visible</h2>
        </div>
        <div class="session-badge">${moduleProgress.totals.attempts ? `${moduleProgress.totals.attempts} reps` : "Fresh run"}</div>
      </div>
      ${atomId ? renderAtomDetail(moduleId, atomId) : ""}
      <div class="list-panel">
        <div class="panel-header">
          <div>
            <p class="section-label">Weakest verbs</p>
            <h3>Current subdeck only</h3>
          </div>
        </div>
        <div class="metric-list">${renderWeakestVerbRows(moduleId)}</div>
      </div>
      <div class="list-panel">
        <div class="panel-header">
          <div>
            <p class="section-label">Weakest persons</p>
            <h3>Rows stay in fixed pronoun order on the heatmap</h3>
          </div>
        </div>
        <div class="metric-list">${renderWeakestPersonRows(moduleId)}</div>
      </div>
...
function renderGrammarBottom(moduleId) {
  return `
    <section class="bottom-grid reveal">
      <article class="keyboard card grammar-heatmap-card">
        <div class="panel-header">
          <div>
      ...
      <article class="leaderboard card grammar-list-card">
        <div class="panel-header">
          <div>
        ...
        <div class="detail-panel">
          <div class="metric-list">${renderWeakestAtoms(moduleId)}</div>
        </div>
      </article>
    </section>
  `;
