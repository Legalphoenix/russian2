/* Russian Skill Coach tight conjugation curriculum.
   Present tense only. First and second conjugation modules.
   Generated for direct use in the existing vanilla JS app.
*/

const CONJUGATION_SHARED_CONFIG = {
  "version": 1,
...
function buildConjugationAtoms(deck, sharedConfig = CONJUGATION_SHARED_CONFIG) {
  const persons = sharedConfig.persons;
  const atoms = [];
  deck.subdecks.forEach((subdeck) => {
    subdeck.verbs.forEach((verb) => {
      persons.forEach((person) => {
        atoms.push({
          id: `${deck.moduleId}__${subdeck.id}__${verb.id}__${person.id}`,
          ...
          answer: verb.forms[person.id],
          fragmentBase: verb.fragmentBaseByPerson[person.id],
          previewSplit: verb.previewSplit,
          contextFrames: verb.contextFrames,
          stageProfileId: deck.defaultStageProfileId,
          ...
          irregularYaBannerText: verb.irregularYaBannerText || null,
          coachNote: verb.coachNote || null,
          regularityTag: verb.regularityTag,
          tags: verb.tags || []
        });
      });
    });
...
function buildTightConjugationCurriculum() {
  return {
    shared: CONJUGATION_SHARED_CONFIG,
    modules: [
      { ...FIRST_CONJUGATION_DECK, atoms: buildConjugationAtoms(FIRST_CONJUGATION_DECK) },
      { ...SECOND_CONJUGATION_DECK, atoms: buildConjugationAtoms(SECOND_CONJUGATION_DECK) }
    ]
  };
}

if (typeof window !== 'undefined') {
  window.CONJUGATION_SHARED_CONFIG = CONJUGATION_SHARED_CONFIG;
  window.FIRST_CONJUGATION_DECK = FIRST_CONJUGATION_DECK;
  window.SECOND_CONJUGATION_DECK = SECOND_CONJUGATION_DECK;
  window.buildConjugationAtoms = buildConjugationAtoms;
  window.buildTightConjugationCurriculum = buildTightConjugationCurriculum;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    CONJUGATION_SHARED_CONFIG,
    FIRST_CONJUGATION_DECK,
    SECOND_CONJUGATION_DECK,
    buildConjugationAtoms,
    buildTightConjugationCurriculum,
  };
}
